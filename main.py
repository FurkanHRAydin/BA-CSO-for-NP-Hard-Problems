...
def main():
    num_cats = 12  # Anzahl der "Katzen" im Schwarm
    ...
    iterations = 1200  # Anzahl der Iterationen für den CSO-Algorithmus

    smp = 4  # Anzahl der erzeugten neuen Lösungen im Seeking Mode
    spc = True  # Berücksichtigt die aktuelle Position im Seeking Mode
    srd = 0.1  # Bereich der Modifikation im Seeking Mode
    cdc = 1    # Anzahl der Dimensionen, die verändert werden (CDC)
    c1 = 2.0  # Gewichtung der besten Position im Tracing Mode
    velocity_limit = 0.1  # Begrenzung der Geschwindigkeit im Tracing Mode
    mr = 0.3  # Wechselwahrscheinlichkeit zwischen Seeking und Tracing Mode
